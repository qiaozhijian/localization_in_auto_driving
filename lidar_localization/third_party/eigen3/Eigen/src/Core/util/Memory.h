...
/** \internal Allocates \a size bytes. If Align is true, then the returned ptr is 16-byte-aligned.
  ...
  */
        ...
        template<>
        EIGEN_DEVICE_FUNC inline void *conditional_aligned_malloc<false>(size_t size) {
            check_that_malloc_is_allowed();
...
/** \internal Destructs the elements of an array.
  ...
  */
        template<typename T>
        EIGEN_DEVICE_FUNC inline void destruct_elements_of_array(T *ptr, size_t size) {
...
void check_size_for_overflow(size_t size) {
...
*
conditional_aligned_new(size_t
size)
{
check_size_for_overflow <T>(size);
...
template<typename T, bool Align> EIGEN_DEVICE_FUNC inline T
*
conditional_aligned_new_auto(size_t
size)
{
if(size==0)
...
inline Index first_aligned(const Scalar *array, Index size) {
    ...
    if (AlignmentSize <= 1) {
        ...
        return std::min<Index>(
                (AlignmentSize - (Index((std::size_t(array) / sizeof(Scalar))) & AlignmentMask)) & AlignmentMask, size);
    }
}
...
struct smart_copy_helper<T, true> {
    EIGEN_DEVICE_FUNC static inline void run(const T *start, const T *end, T *target) { memcpy(target, start,
                                                                                               std::ptrdiff_t(end) -
                                                                                               std::ptrdiff_t(start));
    }
};
...
struct smart_memmove_helper<T, true> {
    static inline void run(const T *start, const T *end, T *target) { std::memmove(target, start, std::ptrdiff_t(end) -
                                                                                                  std::ptrdiff_t(
                                                                                                          start));
    }
};
