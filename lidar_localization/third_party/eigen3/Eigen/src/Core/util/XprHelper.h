...
/** \internal If the template parameter Value is Dynamic, this class is just a wrapper around a T variable that
  ...
  */
        ...
        public:
            ...
            EIGEN_DEVICE_FUNC explicit variable_if_dynamic(T v) {
                EIGEN_ONLY_USED_FOR_DEBUG(v);
                eigen_assert(v == T(Value));
            }

            EIGEN_DEVICE_FUNC static T
...
/** \internal like variable_if_dynamic but for DynamicIndex
  */
        ...
        public:
            ...
            EIGEN_DEVICE_FUNC explicit variable_if_dynamicindex(T v) {
                EIGEN_ONLY_USED_FOR_DEBUG(v);
                eigen_assert(v == T(Value));
            }
...
struct special_scalar_op_base<Derived, Scalar, OtherScalar, BaseType, true> : public BaseType {
    ...
    operator/(const OtherScalar &scalar) const {
        ...
        return CwiseUnaryOp < scalar_quotient2_op < Scalar, OtherScalar >, Derived >
                                                                           (*static_cast<const Derived *>(this), scalar_quotient2_op<Scalar, OtherScalar>(
                                                                                   scalar));
    }
};
...
template<typename A, typename ScalarA, typename ScalarB>
struct cwise_promote_storage_type<A, Dense, scalar_product_op < ScalarA, ScalarB> >      {
typedef A ret;
};
template<typename B, typename ScalarA, typename ScalarB>
